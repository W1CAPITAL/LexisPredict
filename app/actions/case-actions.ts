...
/**
 * Página da carteira (otimização de carteira com 1200+ processos):
 * busca um lote por vez no servidor. `adminView` = visão da empresa inteira
 * (Supervisor/Administrador); caso contrário respeita o filtro do operador.
 */
...
export async function registrarAuditoriaEventAction(
  acao: 'edicao' | 'exclusao' | 'criacao' | 'atendimento' | 'encerramento',
  protocolos: string[],
  detalhes: Record<string, any> = {}
) {
  const { registrarAuditoriaAction } = await import('@/lib/server-db');
