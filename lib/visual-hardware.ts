...
export function applyWallpaperStyle(imageValue: string) {
  ...
  let cssValue = imageValue.trim();
  // Normaliza: URL crua ou data-URL viram url(...); gradiente CSS passa direto
  if (cssValue.indexOf('gradient') === -1 && cssValue.indexOf('url(') !== 0) {
    cssValue = `url(${cssValue})`;
  }
  localStorage.setItem("lexisPredict_wallpaper", imageValue);

  // html = camada de fundo atrás de tudo; body vira vidro translúcido p/ o wallpaper aparecer
  const root = document.documentElement;
  root.classList.add('lexis-wallpaper-active');
  root.style.backgroundImage = cssValue;
