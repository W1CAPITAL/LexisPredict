...
export const MODELOS_DE_PECAS: ModeloPeca[] = [
  ...
  {
    id: 'peticao-revisional-contrato',
    categoria: 'Petições',
    titulo: 'Petição inicial — ação revisional de contrato',
    descricao:
      'Inicial revisional (juros, tarifas, venda casada, encargos) com pedidos e tutela.',
    campos: [
      'cliente',
      'cpfCliente',
      'rgCliente',
    ...
    render: (m) =>
      [
        ...
        m.comarca ? `Comarca de ${seg(m, 'comarca', '')}` : '',
        '',
        'AÇÃO REVISIONAL DE CONTRATO C/C REPETIÇÃO DE INDÉBITO E TUTELA DE URGÊNCIA',
        '',
        `Autor(a): ${qualificaCliente(m)}.`,
        '',
        `Réu: ${nomeSeg(m, 'banco', '[INSTITUIÇÃO FINANCEIRA]')}${
          m.cnpjBanco ? `, CNPJ ${seg(m, 'cnpjBanco', '')}` : ''
        }.`,
        '',
        `Contrato / operação n.º ${seg(m, 'protocolo', '[NÚMERO]')}${
          m.valorContrato ? `, valor de referência ${seg(m, 'valorContrato', '')}` : ''
        }${m.tipoAcao ? `, natureza: ${seg(m, 'tipoAcao', '')}` : ''}.`,
        '',
        'I — DOS FATOS',
        m.resumo ||
          'O(A) autor(a) celebrou com a ré contrato de crédito/financiamento e, no curso da execução, identificou cláusulas e cobranças abusivas (taxa de juros remuneratórios em descompasso com a média de mercado do mesmo produto, tarifas administrativas, eventual venda casada de seguros/produtos e/ou encargos de mora em cumulação indevida), nos termos do CDC.',
        '',
        'II — DO DIREITO',
        'Aplicam-se o Código de Defesa do Consumidor (Lei 8.078/1990), em especial os arts. 6º, 39 e 51, e a jurisprudência do STJ sobre revisão de cláusulas abusivas, tarifas bancárias e limites de encargos. Destaca-se a Súmula 539/STJ quanto à capitalização (quando expressamente pactuada em contratos do SFN a partir de 31/03/2000), devendo a análise observar o que foi efetivamente pactuado e cobrado.',
        '',
        'III — DA TUTELA DE URGÊNCIA',
        'Requer-se tutela de urgência para obstar negativação indevida e/ou busca e apreensão, bem como para autorizar o depósito judicial da parcela incontroversa, se o caso, preservando o bem e o nome do(a) autor(a) até o julgamento.',
        '',
        'IV — DOS PEDIDOS',
        'Requer:',
        'a) a citação da ré para contestar, sob pena de revelia;',
        'b) a concessão da tutela de urgência nos termos acima;',
        'c) a exibição do contrato, CET, planilha de evolução do débito e comprovantes de tarifas/seguros (CDC art. 6º, VIII; CPC arts. 396 e ss.);',
        'd) a revisão/anulação das cláusulas e cobranças abusivas, com recálculo do saldo;',
        'e) a repetição do indébito, em dobro quando cabível;',
        'f) a condenação da ré em custas e honorários advocatícios;',
        'g) a produção de todos os meios de prova em direito admitidos.',
        '',
        'Dá-se à causa o valor de R$ ________ (preencher).',
        '',
        'Termos em que pede deferimento.',
        '',
        localData(m),
  ...
  {
    id: 'peticao-tutela-revisional',
    categoria: 'Petições',
    titulo: 'Petição — tutela / depósito de parcela incontroversa',
    descricao:
      'Pedido de liminar em revisional (não negativar / não apreender / depósito).',
    campos: [
      'protocolo',
      'cliente',
      'banco',
    ...
    render: (m) =>
      [
        ...
        '',
        `${nomeSeg(m, 'cliente', '[AUTOR(A)]')}, nos autos da ação revisional em face de ${nomeSeg(m, 'banco', '[RÉ]')}, requer a concessão de tutela de urgência para:`,
        '',
        'a) obstar a inscrição do nome do(a) autor(a) em cadastros de inadimplentes em razão do contrato discutido;',
        'b) obstar a busca e apreensão / consolidação da propriedade do bem, se houver;',
        m.valorProposta
          ? `c) autorizar o depósito judicial mensal da quantia de ${seg(m, 'valorProposta', '')}, correspondente à parcela incontroversa estimada;`
          : 'c) autorizar o depósito judicial da parcela incontroversa, a ser indicada em planilha anexa;',
        '',
        m.resumo
          ? `Fundamentos: ${m.resumo}`
          : 'Presentes a probabilidade do direito e o perigo de dano, nos termos do art. 300 do CPC.',
        '',
        'Termos em que pede deferimento.',
        '',
        localData(m),
