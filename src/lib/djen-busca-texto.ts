/**
 * Consulta DJEN por teor (texto) — Busca e Apreensão.
 * Isolado de fetchDjenComunicacoes (por CNJ).
 */
  ...
  try {
    ...
    if (response.status === 403) {
      return {
        success: false,
        isGeoBlocked: true,
        error: 'DJEN geo-bloqueou o servidor (403). Vercel deve estar em gru1 (São Paulo).',
        count: 0,
        items: [],
      };
    }
    ...
    return {
      success: false,
      error: e?.message || 'Falha na consulta DJEN.',
      count: 0,
      items: [],
    };
  }
