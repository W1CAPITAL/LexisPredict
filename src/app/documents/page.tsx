...
import {
  FileText, Zap, Loader2, Edit3, Upload, FileUp, Shield,
  CheckCircle2, Building2, AlertCircle, MapPin, User, Trash2,
  Eye, Calendar, Fingerprint, Hash, Settings, Briefcase, Globe,
  FileSignature, Files, Scale, Library, ArrowRight, Repeat
} from 'lucide-react';
...
export default function DocumentGenerator() {
  ...
  const handleExtract = async () => {
    ...
    try {
      const res = await extrairDadosProcuracaoAction(
        inputText,
        selectedLawyer?.nome || '',
        selectedState
      );
      ...
      if (res.success) {
        setExtractedData({
          ...
          local: selectedState,
          dataExtenso: `${selectedState}, ${new Date().toLocaleDateString('pt-BR', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
          })}`
      ...
      const payload = {
        ...
        advogado: {
          ...
          nacionalidade: selectedLawyer.nacionalidade ||
            (selectedLawyer.genero === 'F' ? 'brasileira' : 'brasileiro'),
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-16 border-b border-[#dddbda] bg-white flex items-center justify-between px-8 shrink-0 z-40">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-black text-xl text-black uppercase tracking-tighter">
              Gerador de Procurações
            </h1>
          </div>
          <Badge
            variant="outline"
            className="border-black border-2 text-black font-black uppercase text-[10px]"
          >
            Cloud Repository
          </Badge>
        </header>
          ...
          <div className="flex-1 overflow-auto p-4 lg:p-8 max-w-7xl mx-auto w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                <p className="text-[10px] font-bold uppercase text-black/40 mb-3">
                  Ad Judicia · triagem neural · selar PDF
                </p>
                          ...
                          <div className="space-y-2">
                            ...
                            <Select value={selectedLawyerId} onValueChange={(val) => {
                              ...
                              <SelectContent className="bg-white border-2 border-black rounded-none">
                                {banca.map((adv) => (
                                  <SelectItem key={adv.id} value={adv.id} className="font-black uppercase text-[10px]">
                                    {adv.nome}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            ...
                            <Select value={selectedState} onValueChange={setSelectedState}>
                              ...
                              <SelectContent className="bg-white border-2 border-black rounded-none">
                                {selectedLawyer && Object.keys(selectedLawyer.oabs || {}).map((uf) => (
                                  <SelectItem key={uf} value={uf} className="font-black uppercase text-[10px]">
                                    {uf}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                    ...
                    <Card className="bg-white border-2 border-black rounded-none shadow-[8px_8px_0px_#000]">
                      ...
                      <CardContent className="p-6">
                        <div
                          ...
                          className="border-2 border-dashed border-black/20 p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-black group transition-all"
                        >
                          {fileLoading
                            ? <Loader2 className="animate-spin text-black" size={32} />
                            : <FileUp size={48} className="text-black/20 group-hover:text-white mb-4" />}
                          <input
                            type="file"
                            accept=".pdf"
                            className="hidden"
              ...
              <div className="space-y-8 animate-in fade-in duration-500 max-w-5xl mx-auto pb-20">
                <div className="flex items-center justify-between border-b-2 border-black pb-4">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={() => setStep(1)}
                    className="font-black uppercase text-[10px] border-2 border-black rounded-none"
                  >
                    Voltar ao Upload
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <Card className="bg-white border-2 border-black rounded-none shadow-[6px_6px_0px_#000]">
                    <CardHeader className="bg-[#f8f9fb] border-b-2 border-black py-3">
                      <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                        <User size={14} /> Dados do Outorgante
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="p-6 space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-1">
                          ...
                          <Input value={extractedData.cliente.cpf} onChange={(e) => updateField('cliente', 'cpf', e.target.value)} className="border-black font-black uppercase rounded-none" />
                        </div>
                        <div className="grid gap-1">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-1">
                          ...
                          <Input value={extractedData.cliente.email} onChange={(e) => updateField('cliente', 'email', e.target.value)} className="border-black font-bold lowercase rounded-none" />
                        </div>
                        <div className="grid gap-1">
                          <Label className="text-[9px] font-black uppercase">Telefone</Label>
                          <Input value={extractedData.cliente.telefone} onChange={(e) => updateField('cliente', 'telefone', e.target.value)} className="border-black font-bold rounded-none" />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-1">
                          <Label className="text-[9px] font-black uppercase">Estado Civil</Label>
                          <Input value={extractedData.cliente.estadoCivil} onChange={(e) => updateField('cliente', 'estadoCivil', e.target.value)} className="border-black font-bold uppercase rounded-none" />
                        </div>
                        <div className="grid gap-1">
                          <Label className="text-[9px] font-black uppercase">Nacionalidade</Label>
                          <Input value={extractedData.cliente.nacionalidade} onChange={(e) => updateField('cliente', 'nacionalidade', e.target.value)} className="border-black font-bold uppercase rounded-none" />
                        </div>
                      </div>
                      <div className="grid gap-1">
                        <Label className="text-[9px] font-black uppercase">
                          <Briefcase size={10} className="inline mr-1"/> Profissão
                        </Label>
                        <Input value={extractedData.cliente.profissao} onChange={(e) => updateField('cliente', 'profissao', e.target.value)} className="border-black font-bold uppercase rounded-none" />
                      </div>
                      <div className="grid gap-1">
                        <Label className="text-[9px] font-black uppercase flex items-center gap-1.5">
                  ...
                  <div className="space-y-6">
                    <Card className="bg-white border-2 border-black rounded-none shadow-[6px_6px_0px_#000]">
                      <CardHeader className="bg-[#f8f9fb] border-b-2 border-black py-3 flex flex-row items-center justify-between">
                        <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                          <Building2 size={14} /> Dados do Processo
                        </CardTitle>
                        <div className="flex items-center gap-4">
                ...
                {/* CORREÇÃO DE BUILD:
                    O <p> anterior estava aberto antes do componente React,
                    fazendo o parser interpretar "banca" como texto inválido.
                    O painel agora é um irmão do parágrafo de preview. */}
                <Card className="bg-white border-2 border-black rounded-none shadow-[8px_8px_0px_#000] overflow-hidden">
                  <CardHeader className="bg-[#f8f9fb] border-b-2 border-black py-3 flex flex-row items-center justify-between">
                    <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                      <Eye size={14} /> Preview Visual
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-10 text-black font-serif text-[11pt] leading-relaxed bg-white space-y-6">
                    <h3 className="text-center font-bold uppercase underline">
                    ...
                    <p className="text-justify indent-10">
                      <strong>{extractedData.cliente.nome.toUpperCase()}</strong>, {extractedData.cliente.nacionalidade}, {extractedData.cliente.estadoCivil}, {extractedData.cliente.profissao}, portador do RG sob Nº {extractedData.cliente.rg} e CPF sob Nº {extractedData.cliente.cpf}, residente em {extractedData.cliente.endereco}, nomeia como seu procurador <strong>{selectedLawyer?.nome.toUpperCase()}</strong>, {selectedLawyer?.nacionalidade || ''}, {selectedLawyer?.estado_civil || selectedLawyer?.estadoCivil || ''}, OAB/{selectedState} {selectedLawyer?.oabs[selectedState]}, com endereço profissional em {selectedLawyer?.endereco}.
                    </p>
                  </CardContent>
                </Card>
