/**
 * @fileOverview Painel de operações de dados — Superadmin only
 * Estilo similar à lista de Processos: buscar, apagar com confirmação, editar JSON leve.
 * Rota: /settings/ops
 */
"use client";
...
export default function SettingsOpsPage() {
  ...
  const loadTables = useCallback(async () => {
    ...
    if (res.success) {
      ...
      if (res.tables.length && !res.tables.includes(table)) {
        setTable(res.tables[0]);
      }
    } else {
  ...
  return (
    <div className="flex h-screen bg-transparent font-sans text-foreground overflow-hidden relative z-10">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden glass-panel">
        <header className="h-auto border-b border-border/50 bg-card/60 backdrop-blur-xl flex items-center justify-between p-4 sm:px-8 shrink-0 gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mt-0.5">
                Superadmin · whitelist de tabelas · apagar com confirmação
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Input
              placeholder="Buscar na lista carregada..."
              value={search}
      ...
      {/* Confirmar apagar — igual espírito da aba Processos */}
      <Dialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <DialogContent className="rounded-xl border-2 border-black max-w-md">
          ...
          <p className="text-sm text-muted-foreground">
            Remover permanentemente o registro <span className="font-mono font-bold">{deleteId}</span> da
            tabela <span className="font-black">{table}</span>? Esta ação não pode ser desfeita pelo app.
          </p>
