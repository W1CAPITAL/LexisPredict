/**
 * BACEN SGS — API pública gratuita (sem chave, sem CNPJ).
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const dataRef = String(req.query.data || '').trim(); // YYYY-MM or DD/MM/YYYY
  ...
  try {
    ...
    return res.status(200).json({
      ...
      taxaMediaAnualApprox: taxaMedia != null ? Number((Math.pow(1 + taxaMedia / 100, 12) - 1) * 100).toFixed(2) : null,
      fonte: 'Banco Central do Brasil — SGS (público, gratuito)',
      pontos: rows.slice(-6),
    });
  } catch (e: any) {
